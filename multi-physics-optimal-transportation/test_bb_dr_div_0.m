...
switch test
    ...
    case 'shape'
        f0 = normalize(double(imread('shape_1.png')));
        f0 = normalize(f0(:,:,1));
        f1 = normalize(double(imread('shape_65.png')));
        f1 = normalize(f1(:,:,1));
    case 'obstacle'
...
f_init(:,:,2) = 0;
% prox operators
proxJeps    = @(U,V,gamma)deal(div_proj(U), proxJ(V,gamma,epsilon,alpha,obstacle));
proxI       = @(U,V,gamma)deal((U), V);
