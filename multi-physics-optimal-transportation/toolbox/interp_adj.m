...
switch(length(d))
 ...
 case 3
   %% 3D field %%
   U.M{1}  = cat(1,2*V(1,:,:,1),V(1:end-1,:,:,1)+V(2:end,:,:,1),2*V(end,:,:,1));
   U.M{2}  = cat(2,2*V(:,1,:,2),V(:,1:end-1,:,2)+V(:,2:end,:,2),2*V(:,end,:,2));
   U.M{3}  = cat(3,2*V(:,:,1,3),V(:,:,1:end-1,3)+V(:,:,2:end,3),2*V(:,:,end,3)); 
 case 4
   %% 4D field %%
   U.M{1}  = cat(1,V(1,:,:,:,1),V(1:end-1,:,:,:,1)+V(2:end,:,:,:,1),V(end,:,:,:,1));
   U.M{2}  = cat(2,V(:,1,:,:,2),V(:,1:end-1,:,:,2)+V(:,2:end,:,:,2),V(:,end,:,:,2));
